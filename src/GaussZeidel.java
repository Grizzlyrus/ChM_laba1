...
public class GaussZeidel {
    ...
    public GaussZeidel(Matrix A, Matrix b,double epsilon){
        this.A=A;
        this.b=b;
...
//        double[][] b1 = b.getArrayCopy();
        for (int j = 0; j < b.getColumnDimension(); j++) {
            ...
            do {
                ...
                for (int i = 0; i < m; i++) {
                    for (int k = 0; k < i ; k++) {
                        xnew[i][j] -= A1[i][k] / A1[i][i] * xnew[k][j];
                        number+=3;
                    }
    ...
    public boolean CanSolve(){
        ...
        double[][] minusLplusD = A.getArrayCopy();
        double[][] U = A.getArrayCopy();
        for (int i = 0; i<m; i++){
            for (int k = 0; k<m; k++){
                if(i>=k){
                    minusLplusD[i][k] = -minusLplusD[i][k];
                    U[i][k] = 0;
                }else{
                    minusLplusD[i][k] = 0;
                }
        ...
        Matrix result = new Matrix(minusLplusD).inverse(0).times(new Matrix(U));
